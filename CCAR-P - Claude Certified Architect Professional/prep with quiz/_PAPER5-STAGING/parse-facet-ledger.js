#!/usr/bin/env node
/*
 * Parses ../FACET-LEDGER.md (now including the 2026-09-01 D2 corpus expansion, 39 D2 facets)
 * into structured facet + misconception-unit rows, cross-references with prior-papers-
 * analysis.json (ground truth of what Papers 1-4 actually shipped, per this session's own §1.7
 * extraction) to compute remaining fresh supply per section/domain. Same method as
 * _PAPER4-STAGING/parse-facet-ledger.js, extended to Papers 1-4.
 */
const fs = require("fs");
const path = require("path");
const scriptDir = __dirname;

const ledger = fs.readFileSync(path.join(scriptDir, "../FACET-LEDGER.md"), "utf8").split("\n");
const priorPapers = JSON.parse(fs.readFileSync(path.join(scriptDir, "prior-papers-analysis.json"), "utf8"));
...
fs.writeFileSync(
  path.join(scriptDir, "facet-supply.json"),
...
console.log(
  "\nD2 sections with NO fresh facet:",
