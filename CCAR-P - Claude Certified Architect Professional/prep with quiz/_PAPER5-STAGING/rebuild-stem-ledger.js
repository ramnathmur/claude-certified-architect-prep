#!/usr/bin/env node
/* Appends Paper 5's 63 stems to STEM-LEDGER.md, matching the Paper 1-4 append format exactly. */
const fs = require("fs");
...
let table = "\n### Paper 5 — appended 2026-09-01\n\n";
table += "| id | src | domain | words | excerpt | token signature |\n";
table += "|---|---|---|---|---|---|\n";
items.forEach((it) => {
  const id = `P5-${String(it.g).padStart(2, "0")}`;
  const words = it.stem.split(/\s+/).length;
  table += `| \`${id}\` | Paper 5 | ${it.domain} | ${words} | ${excerpt(it.stem)} | \`${sig(it.stem)}\` |\n`;
});
...
console.log("Appended 63 Paper 5 rows to STEM-LEDGER.md");
