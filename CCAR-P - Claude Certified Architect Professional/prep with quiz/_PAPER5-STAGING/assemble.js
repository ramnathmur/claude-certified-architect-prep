#!/usr/bin/env node
/*
 * Paper 5 assembly. Loads all 13 sub-batch files, validates against plan.json, computes
 ...
 * Ported unchanged from _PAPER4-STAGING/assemble.js except the batch-file list.
 */
const fs = require("fs");

const BATCH_FILES = [
  "p5-d1-batch1-items.json", "p5-d1-batch2-items.json", "p5-d2-batch1-items.json", "p5-d2-batch2-items.json",
  "p5-d3-batch1-items.json", "p5-d3-batch2-items.json", "p5-d4-batch1-items.json", "p5-d4-batch2-items.json",
  "p5-d5-batch1-items.json", "p5-d5-batch2-items.json", "p5-d6-batch1-items.json", "p5-d6-batch2-items.json",
  "p5-d7-items.json",
];
...
for (const f of BATCH_FILES) {
  if (!fs.existsSync(f)) throw new Error(`${f}: not written yet`);
...
// Verify direction/shape match the plan exactly
