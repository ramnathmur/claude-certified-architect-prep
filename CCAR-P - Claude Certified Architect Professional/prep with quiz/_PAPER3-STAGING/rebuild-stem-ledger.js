#!/usr/bin/env node
/* Appends Paper 3's 63 stems to STEM-LEDGER.md, matching the Paper 1/Paper 2 append format exactly. */
const fs = require("fs");
...
let table = "\n### Paper 3 — appended 2026-08-31\n\n";
table += "| id | src | domain | words | excerpt | token signature |\n";
table += "|---|---|---|---|---|---|\n";
items.forEach((it) => {
  const id = `P3-${String(it.g).padStart(2, "0")}`;
  const words = it.stem.split(/\s+/).length;
  table += `| \`${id}\` | Paper 3 | ${it.domain} | ${words} | ${excerpt(it.stem)} | \`${sig(it.stem)}\` |\n`;
});
...
console.log("Appended 63 Paper 3 rows to STEM-LEDGER.md");
