<!--
  CCA-F Mock Exam Orchestration Prompt — v10
  Author persona: Learning Systems Architect
  Changes from v9 (CLAUDE.md v2.3→v2.4 — live running-accuracy percentage,
  2026-07-13):
    - Phase 4.f build spec: sticky nav now also requires a live running-
      accuracy percentage next to the score pill — round(correct/answered×100),
      "—%" when answered=0. Color-coded red below / green at-or-above the
      pass-equivalent raw threshold 620/900 = 31/45 ≈ 68.89% (derived from
      round((correct/60)×900+100) ≥ 720), not a round number. Compared against
      the unrounded running ratio, not the display value.
    - self_verification exam content checks gained one new line item requiring
      this element and its correct threshold in the shipped HTML.
    - Applies to Exam 7 onward — Exams 1–6 are historical record and are not
      retrofitted.
...
with elapsed timer, a live running-accuracy percentage, and a collapsible
...
f. Build the HTML file at `mock-exams/CCA-Prep_MockTest-N_v1.html` (relative to
   ...
   v2.4 Step 6 for naming/placement and v2.4 Step 5 for the HTML spec below:
   ...
   unobtrusive elapsed timer, a live running-accuracy percentage next to the
   score pill (round(correct/answered×100), "—%" when answered=0; red below /
   green at-or-above the pass-equivalent raw threshold 620/900 = 31/45 ≈
   68.89%, compared against the unrounded running ratio — see CLAUDE.md v2.4
   Step 5 item 3), and a collapsible jump-map (per-block rows of numbered
   chips, answered/unanswered/current states, "Show results now" link),
   selection-aware rationale panel
...
## 4. Exam HTML File
Self-contained HTML: CCA-Prep_MockTest-N_v1.html per CLAUDE.md v2.4 Step 5.
...
☐ Phase 4.e.6 Fidelity Verification Gate report produced — all six checks
   ...
   4.f began
☐ Sticky nav shows a live running-accuracy percentage next to the score pill
   (round(correct/answered×100), "—%" when answered=0), color-coded red/green
   against the pass-equivalent raw threshold 620/900 ≈ 68.89% (not a round
   number), compared against the unrounded running ratio — verified live in
   browser at 0, mid-exam, and 60/60 answered states (CLAUDE.md v2.4 Step 5
   item 3)
