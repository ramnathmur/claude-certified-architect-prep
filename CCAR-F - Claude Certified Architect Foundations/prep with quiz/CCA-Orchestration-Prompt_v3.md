<!--
  CCA-F Mock Exam Orchestration Prompt — v3
  Author persona: Learning Systems Architect
  Purpose: Adaptive, self-improving MCQ exam generation for the Anthropic
           Claude Certified Architect — Foundations (CCA-F) exam.
  What's new in v3 vs v2:
    - GENERATION-INTELLIGENCE.md: new AI-to-AI feedback loop file
    - State injection Step 2.5: load generation intelligence at session start
    - Phase 4b.5: consume intelligence before question selection
    - Phase 5a.5: write updated intelligence after generation
    - Self-verification: 4 new checklist items covering the intelligence layer
-->
...
## Generation intelligence file (new in v3)
...
Apply exactly one of these four branches:
  ...
  BRANCH D — SESSION-STATE.md exists, status = "IN_PROGRESS", and EXAM-LOG.md
    ...
    EXAM-LOG.md skeleton entry and DASHBOARD-DATA.jsonl line for that exam NOW,
    before any new work. Announce: "Recovering interrupted session: completing
    log entry for [exam_file] before proceeding."

### Step 2.5 — Read GENERATION-INTELLIGENCE.md (AI-to-AI learning)
If the file does not exist, create it using the empty template defined in
Phase 5a.5 and note: "GENERATION-INTELLIGENCE.md not found — created fresh.
This session will bootstrap the intelligence layer."

If the file exists, extract and hold in working memory:
- kd_coverage        : which of the 25 Key Distinctions have been used, in which
                       exams, and their associated learner performance notes
...
c. IDENTIFY weakest domain: lowest % correct. List all if tied.
...
e. CONFIRMED WEAKNESS CHECK:
   ...
   If same domain was weakest in both:
...
b.5 APPLY GENERATION INTELLIGENCE (consume GENERATION-INTELLIGENCE.md):
   Before selecting any question, apply these rules from loaded intelligence:

   Coverage rules:
   - Skip Key Distinctions already used in prior exams unless all 25 have been
     used (full cycle complete), in which case begin a second cycle and note it.
   ...
   If GENERATION-INTELLIGENCE.md was just bootstrapped (first session): skip
   this step and proceed with default selection.

c. Apply confirmed-weakness adjustment (if confirmed_weakness = true):
   Confirmed domain gets +2 questions.
   Remove 1 from D2 and 1 from D5 (lowest-weight domains).
   State adjusted distribution in Session Close Summary.

d. Cycle through Key Distinctions (CCA-Prep_Key-Distinctions_v1.md) in priority
   order: unused traps first (per kd_coverage from intelligence file), then
   partially-cycled traps, then fully-cycled. Each trap seeds one question.
   Fill remaining quota from corpus sections not yet seeded by a Key Distinction.
...
f. Build the HTML file: CCA-Prep_MockTest-N_v1.html
   ...
   - Prior performance:
       ...
       exams_scored ≥ 1, last exam is immediately prior
                           → "Last score: [X/30] — est. [XXX]/1000"
       exams_scored ≥ 1, last scored exam is not the immediately prior
                           → "Last scored: Exam [M] — [X/30] — est. [XXX]/1000"
   - "Weakest domain: [DX | 'No data yet']"
   - "Focus today: [confirmed_weakness_domain | 'Full coverage']"
   - If an Insights Round was generated this session:
       "Study recommendation: [domain] — see Insights Round [N] in EXAM-LOG.md"
...
a. APPEND skeleton exam entry to EXAM-LOG.md. Must include:
...
| # | Distinction Title | Domain | Used In Exam(s) | Learner Signal | Notes |
|---|---|---|---|---|---|
| 1 | [title] | [DX] | [Exam N, ...] | [strong/weak/unknown] | [observation] |
…
...
<!-- Options that were too easy to reject by elimination — never reuse verbatim -->
...
- Corpus gaps noticed: [bullet — concept in guide_en.MD not yet in corpus, or None]
- Blueprint ambiguity: [bullet — any CLAUDE.md rule that was hard to apply, or None]
- Recommended next action: [bullet — corpus update? blueprint clarification? None]
─────────────────────────────────────────────────────────────

For this session's update:
  i.  KD Coverage Tracker: add this exam's questions to the relevant rows.
      For each Key Distinction used, note which exam and any learner signal
      available from EXAM-LOG.md (the cross-reference from Phase 2d if scoring
      ran, or "unknown" if no score yet).
  ...
  iii.Distractor Quality: review each distractor written this session.
      ...
      corpus. Add to the Distractor Quality Notes list.
...
- Active confirmed weakness (if any)
...
☐ EXAM-LOG.md updated — skeleton appended (and/or score entry completed if
   Phase 2 ran)
☐ Skeleton entry includes ALL 30 question stems in the deduplication section
...
<!-- Generation intelligence checks (new in v3) -->
☐ GENERATION-INTELLIGENCE.md has been overwritten with the full updated version
☐ KD Coverage Tracker updated for all Key Distinctions used in this exam
...
☐ At least 1 question uses an effective pattern from the pattern library
   (unless the library was just bootstrapped this session)
