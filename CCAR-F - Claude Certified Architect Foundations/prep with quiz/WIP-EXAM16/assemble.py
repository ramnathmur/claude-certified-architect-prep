...
# ---- block x domain allocation vs the pre-planned table ----
PLAN = {
    0: {"D1": 5, "D2": 3, "D3": 2, "D4": 2, "D5": 3},
    1: {"D1": 4, "D2": 3, "D3": 2, "D4": 2, "D5": 4},
    2: {"D1": 4, "D2": 4, "D3": 4, "D4": 3, "D5": 0},
    3: {"D1": 3, "D2": 1, "D3": 4, "D4": 5, "D5": 2},
}
for b in range(4):
...
for b in range(4):
    ...
    if got != PREPLAN[b]:
        errs.append(f"block {b+1} letter sequence {got} != pre-plan {PREPLAN[b]}")
...
for b in range(4):
    seq = "".join(chr(65 + q["correct"]) for q in questions
                  if q["block"] == b and not q.get("selectN"))
    tally = collections.Counter(seq)
    print(f"  block {b+1}: {seq}  {dict(sorted(tally.items()))}")

# ---- corpus section coverage ----
import re
...
print(f"\ndistinct corpus sections cited as whyRight: {len(secs)}")
dupes = {k: v for k, v in secs.items() if v > 1}
print(f"sections used more than once: {dupes or 'none'}")
...
out = os.path.join(HERE, "exam16-data.json")
