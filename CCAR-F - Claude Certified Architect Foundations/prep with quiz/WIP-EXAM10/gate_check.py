...
PRIMARY = {
    ...
    1: {"D3", "D5"},         # Code Generation with Claude Code
    2: {"D1", "D2", "D5"},   # Multi-Agent Research System
    3: {"D4", "D5"},         # Structured Data Extraction
}

blocks = []
for f in BLOCKS:
    blocks.append(json.load(open(f"WIP-EXAM10/{f}", encoding="utf-8")))
...
cite_map = {}
for q in all_q:
    cites = [q["whyRight"]["cite"]] + [w["cite"] for w in q["whyWrong"]]
    for c in cites:
        # normalize: take primary "Dn §n.n" token
        m = re.search(r"D\d\s*§\d+\.\d+", c)
        key = m.group(0) if m else c
        cite_map.setdefault(key, []).append(q["g"])

# whyRight-only collisions (the meaningful ones - same section as PRIMARY test twice)
primary_cite_map = {}
for q in all_q:
    m = re.search(r"D\d\s*§\d+\.\d+", q["whyRight"]["cite"])
...
# ---------- Jaccard near-duplicate scan (stem-level, exam-internal) ----------
