...
- **Exam posture:** this is the one finding that makes a corpus statement affirmatively false. `CCA-Prep_Exam-Mechanics_v2.md` currently states "Multiple choice — 1 correct + 3 incorrect options, single answer" as an official fact. **Corpus edit CG-03 proposed and approved same-session, 2026-08-09** (see GENERATION-INTELLIGENCE.md → Pending Corpus Decisions — closed, not pending). *(Corrected 2026-08-30 via `/sync-up`; this line said "awaiting sign-off" three weeks after approval.)* Note the generator's own 4-option single-answer format is a separate, deliberate product decision and is NOT automatically implicated.
